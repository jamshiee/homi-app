...
export default function Step7Review() {
  ...
  const handlePublish = async () => {
    ...
    setSubmitStep('Creating property listing...');

    try {
      // 1. Core Property & details validation payload
      const payload = {
        title,
        type,
        ...
        latitude: parseFloat(latitude) || 11.0510,
      ...
      // 2. Sequential Multipart Media Form Uploads
      if (photos && photos.length > 0) {
        for (let i = 0; i < photos.length; i++) {
          setSubmitStep(`Uploading photo ${i + 1} of ${photos.length}...`);
          const p = photos[i];

          // Compress before upload to avoid tunnel timeouts
          ...
          // Use uploadClient (2-min timeout) instead of apiClient (10s timeout)
      ...
      Toast.show({
        ...
        text2: 'Your property is now active on the feed.',
      });

      // 3. Reset form and redirect to Home feed
      await resetForm();
      router.replace('/(tabs)');
    } catch (err: any) {
      console.error('Publish error:', err);
      const msg = err.response?.data?.message || err.message || 'Check inputs and try again.';
        ...
        return (
          <View style={styles.specGrid}>
            <View style={styles.specBox}>
              ...
              <Text style={styles.specLabel}>{landDetail?.areaUnit?.toUpperCase()}</Text>
            </View>
          </View>
        );
      case 'house':
        return (
          <View style={styles.specGrid}>
            ...
            <View style={styles.specBox}>
              <Text style={styles.specVal}>{houseDetail?.furnishingStatus?.replace('_', ' ').toUpperCase()}</Text>
        ...
        return (
          <View style={styles.specGrid}>
            <View style={styles.specBox}>
              ...
              <Text style={styles.specLabel}>{buildingDetail?.areaUnit?.toUpperCase()}</Text>
            </View>
            <View style={styles.specBox}>
              <Text style={styles.specVal}>{buildingDetail?.subType?.toUpperCase()}</Text>
              <Text style={styles.specLabel}>SUBTYPE</Text>
            </View>
            <View style={styles.specBox}>
              <Text style={styles.specVal}>{buildingDetail?.currentStatus?.replace('_', ' ').toUpperCase()}</Text>
  ...
  if (isSubmitting) {
    return (
      <View style={styles.loadingContainer}>
        ...
        <Text style={styles.loadingTitle}>Publishing Listing...</Text>
  ...
  return (
    <ScrollView style={styles.container} showsVerticalScrollIndicator={false}>
      ...
      <Text style={styles.subtitle}>Double check all inputs before making this listing active</Text>
      ...
      <View style={styles.card}>
        ...
        {advanceAmount ? (
          <Text style={styles.advanceText}>Advance Deposit: ₹{advanceAmount.toLocaleString()}</Text>
        ) : null}
            ...
            {amenityIds.map((id) => {
              ...
              return (
                <View key={id} style={styles.featurePill}>
                  <MaterialCommunityIcons name={(item.iconName || 'check') as any} size={14} color={Colors.yellow} />
