...
export default function PostScreen() {
  ...
  // Non-Admin Permission Guard
  if (!isAdmin) {
    return (
      <SafeAreaView style={styles.guardContainer}>
        <View style={styles.guardCard}>
          <View style={styles.errorIcon}>
            <MaterialCommunityIcons name="shield-lock-outline" size={60} color={Colors.error} />
          </View>
          <Text style={styles.guardTitle}>Admin Access Required</Text>
          <Text style={styles.guardSubtitle}>
            Only authorized administrator and lister accounts can create and publish listings on Homi.
          </Text>
        </View>
      </SafeAreaView>
    );
  ...
  const isNextDisabled = () => {
    switch (step) {
      ...
      case 3:
        ...
        if (type === 'building') return !buildingDetail?.totalArea || !buildingDetail?.subType;
        if (type === 'hotel') return !hotelDetail?.subType || !hotelDetail?.roomType;
        return true;
      case 4:
    ...
    <SafeAreaView style={styles.container} edges={['top', 'left', 'right']}>
      ...
      <View style={styles.header}>
        <View>
          ...
          <Text style={styles.headerSubtitle}>Publish property to the Homi feed</Text>
        </View>
        <TouchableOpacity style={styles.resetButton} onPress={() => void resetForm()}>
          <MaterialCommunityIcons name="refresh" size={20} color={Colors.lightMuted} />
      ...
      <View style={styles.progressContainer}>
        ...
        <View style={styles.progressLabels}>
          ...
          <Text style={styles.progressPercent}>{Math.round((step / 7) * 100)}% Complete</Text>
        </View>
      </View>
